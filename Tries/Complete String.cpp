    ...
    void insert(string word) {
        ...
        for (int i = 0; i < word.size(); i++) {
            if (!node->ContainsKey(word[i])) {
                node->put(word[i], new Node()); //if node not exists means then we can insert the value with referance of new Node().
            }
    ...
    for (auto it : words) {
        if (trie.checkIfPrefixExists(it)) {
            if (it.size() > longest.size()) {
                longest = it;
            } else if (it.size() == longest.size() && it < longest) {  //lexicographically checking condition
                longest = it;
            }
        }
    }
